...
function selectpickerDirective($parse, $timeout) {
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            var spThemeObserveHandler = attrs.$observe('spTheme', function (val) {
                $timeout(function () {
            ...
            var ngModelWatchHandler = angular.noop, ngDisabledWatchHandler = angular.noop;
            if (attrs.ngModel) {
                ngModelWatchHandler = scope.$watch($parse(attrs.ngModel), refresh, true);
            }

            if (attrs.ngDisabled) {
                ngDisabledWatchHandler = scope.$watch($parse(attrs.ngDisabled), refresh, true);
            }

            scope.$on('$destroy', function () {
                spThemeObserveHandler();
                ngModelWatchHandler();
                ngDisabledWatchHandler();
                $timeout(function () {
                    element.selectpicker('remove');
                }, 0, false);
            });
        }
    };
